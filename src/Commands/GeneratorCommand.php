...
abstract class GeneratorCommand extends Command implements PromptsForMissingInput
{
    ...
    protected string $migrationNamespace = 'database\migrations';

    protected string $migrationFields = '';

    protected array $modelFillable = [];

    protected string $routeNamespace = 'routes';
    ...
    public function __construct(Filesystem $files)
    {
        ...
        $this->migrationNamespace = config('crud.migration.namespace', $this->migrationNamespace);
    ...
    protected function _getMigrationPath($name, $columns): string
    {
        $migrationSample = [];
        $this->modelFillable = $columns;

        for ($i = 0; $i < count($columns); $i++) {
            $migrationSample[] = '$table->string("'.$columns[$i].'");';
        }
        $this->migrationFields = implode("\n", $migrationSample);
        return base_path($this->_getMigrationTablePath($this->migrationNamespace) . date('Y_m_d_his') . "_create_{$name}_table.php");
    }
    ...
    private function _getMigrationTablePath($namespace): string
    {
        $str = Str::start(Str::finish(Str::after($namespace, 'App'), '\\'), '\\');
    ...
    protected function buildReplacements(): array
    {
        return [
            ...
            '{{migrationName}}' => strtolower($this->table),
            '{{migrationNamespace}}' => $this->migrationNamespace,
            '{{migrationColumn}}' => $this->migrationFields,
        ];
    }
    ...
    protected function getHead($title): string
    {
        ...
        $attr = match ($this->options['stack']) {
            'tailwind', 'livewire' => 'scope="col" class="py-3 pl-4 pr-3 text-left text-xs font-semibold uppercase tracking-wide text-black"',
            default => ''
        };
    ...
    protected function modelReplacements(): array
    {
        ...
        foreach ($this->modelFillable as $column) {
            $properties .= "\n * @property \${$column}";

            if (!in_array($column, $this->unwantedColumns)) {
                $livewireFormProperties .= "\n    public \${$column} = '';";
                $livewireFormSetValues .= "\n        \$this->{$column} = \$this->{$modelName}Model->{$column};";
            }

            if ($column) {
                $rulesArray[$column] = 'required';
            }
        }
        ...
        $fillable = function () {
            $filterColumns = $this->modelFillable;
