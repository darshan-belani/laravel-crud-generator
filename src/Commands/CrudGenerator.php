...
class CrudGenerator extends GeneratorCommand
{
     ...
     * @var string
     */
    protected $description = 'CRUD operations with model migration request views route files';
    ...
    protected function writeRoute(): static
    {
        ...
        foreach ($lines as $line) {
            $this->info('<bg=white;fg=black>'.$line.'</>');
            if ($this->options['stack'] == 'api') {
                $pathToWebFile = base_path('routes/api.php');
            }
            $pathToWebFile = base_path('routes/web.php');
            if (File::exists($pathToWebFile)) {
                // Read the content of web.php
                $content = File::get($pathToWebFile);

                // Modify the content (e.g., add a new line of code)
                $newContent = $content . "".$line."\n";
                // Write the modified content back to web.php
                File::put($pathToWebFile, $newContent);
    ...
    {
       $columns =  $this->promptFormigrationColumns();
        $migrationPath = $this->_getMigrationPath($this->table, $columns);
        $data = explode('_',$migrationPath);
            $this->info('Creating Migration ...');

            // Make the migration attributes and replacement
            $replace = $this->buildReplacements();
    ...
    protected function promptFormigrationColumns(): array
    {
        $name = text(
            label: 'How many field you have to required? (Number only)',
            required: true
        );
